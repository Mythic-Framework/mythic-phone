...
const EMPTY_FORM = { name: '', buyin: '', laps: 1, dnf_start: '', dnf_time: '', countdown: '20' };

interface Props {
  onCreatingChange?: (creating: boolean) => void;
}

export default function RedlineTracks({ onCreatingChange }: Props) {
  ...
  const onCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await (await Nui.send('CreateRace', { ...formState, track: creating._id })).json();
      closeCreate();
      showAlert(!res?.failed ? 'Race Created' : res.message);
      if (!res?.failed) dispatch({ type: 'I_RACE', payload: { state: true } });
    } catch { closeCreate(); showAlert('Unable To Create Race'); }
  };
  ...
  const canSubmit = !!(formState.name.trim() && formState.buyin && formState.dnf_start && formState.dnf_time);

  // ── CREATE RACE full-page view ──────────────────────────────────────────
  if (creating) {
    return (
      <div style={{ height: '100%', background: '#0a0c10', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 14, scrollbarWidth: 'thin', scrollbarColor: `${T50} transparent` }}>
          ...
          {/* Race Settings */}
          <div style={{ background: T06, border: `1px solid ${T20}`, borderRadius: 12, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={sectionLabel}>Race Settings</div>
          ...
          {/* Buy In & DNF */}
          <div style={{ background: T06, border: `1px solid ${T20}`, borderRadius: 12, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={sectionLabel}>Buy In & DNF Rules</div>
            <TextField
              fullWidth label="Buy In" name="buyin" variant="outlined"
              value={formState.buyin} onChange={onChange}
              ...
              <div style={{ padding: '0 16px 12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
                  ...
                  <button style={btnStyle(inRace)}
                    ...
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.borderColor = T20; }}>
                    <FontAwesomeIcon icon={['fas', 'flag-checkered']} /> Create Race
                  </button>
                </div>
              </div>
            )}
