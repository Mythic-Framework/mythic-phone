...
export default function RedlineRecent() {
              ...
              <div style={{ padding: '0 16px 12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
                  <button
                    ...
                    <FontAwesomeIcon icon={['fas', 'trophy']} /> Race Results
                  </button>
                </div>
              </div>
            )}
        ...
        {selected && (
          <div>
            {Object.keys(selected.racers)
              .filter((r: string) => selected.racers[r].finished)
              .sort((a: string, b: string) => selected.racers[a]?.place - selected.racers[b]?.place)
              .map((name: string) => (
                <div key={name} style={{ padding: '8px 0', borderBottom: `1px solid ${T15}` }}>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 600, fontSize: 14, color: '#fff' }}>
                    #{selected.racers[name].place} {name}
                  </div>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 300, fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>
                    Cash: ${selected.racers[name].reward?.cash} &nbsp;|&nbsp; Fastest: {formatDuration(selected.racers[name].fastest.lapEnd - selected.racers[name].fastest.lapStart)}
                  </div>
                </div>
              ))}
            {Object.keys(selected.racers)
