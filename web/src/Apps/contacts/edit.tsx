...
export default function EditContact() {
  ...
  const [contact, setContact] = useState<any>(contacts.find((c: any) => c._id === id));

  const onChange = (e: any) => {
    const { name, value, checked, type } = e.target;
    setContact((prev: any) => ({ ...prev, [name]: type === 'checkbox' ? checked : value }));
  };
    ...
    <div style={{ height: '100%', background: BG, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: "'Oswald', sans-serif", fontSize: 18, fontWeight: 600, letterSpacing: '0.08em', color: '#fff' }}>
          <FontAwesomeIcon icon={['fas', 'user-pen']} style={{ fontSize: 16, color: T }} />
          Edit Contact
        </span>
        <button
          onClick={() => navigate(-1)}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={cardStyle}>
          ...
          <TextField fullWidth label="Number" name="number" variant="outlined" required
            onChange={onChange} value={contact.number ?? ''} sx={inputSx} />
        </div>
      ...
      <div style={{ flexShrink: 0, display: 'flex', gap: 10, padding: '12px 16px', borderTop: `1px solid ${hexAlpha(T, 0.2)}`, background: 'rgba(8,10,14,0.98)' }}>
        ...
        <button
          ...
          style={{ flex: 1, height: 44, borderRadius: 10, fontSize: 13, fontFamily: "'Oswald', sans-serif", letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', background: T20, border: `1px solid ${T50}`, color: T, transition: 'all 0.18s' }}
