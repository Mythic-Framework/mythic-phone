export const getLoanTypeName = (type: string): string => {
	switch (type) {
		case 'vehicle':
			return 'Vehicle';
		case 'property':
...
export const getLoanIdentifierType = (type: string): string => {
	switch (type) {
		case 'vehicle':
			return 'Vehicle VIN';
		case 'property':
...
export const getActualRemainingAmount = (loanData: any): number => {
...
export const getNextPaymentAmount = (loanData: any, weeksOverride: number = 1): number => {
	if (loanData.Remaining > 0) {
		const interestMult = (100 + loanData.InterestRate) / 100;
