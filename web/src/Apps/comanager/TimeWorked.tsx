...
const parseDutyTime = (dutyTime: any, job: string): string => {
	if (dutyTime && dutyTime[job]) {
		...
		let timeWorked = 0;
		dutyTime[job].forEach((t: any) => {
			if (t.time >= afterTime) timeWorked += t.minutes;
		});
		return timeWorked > 0
			? moment.duration(timeWorked, 'minutes').humanize()
			: '0 minutes';
	}
	return '0 minutes';
};
...
const TimeWorked: React.FC<Props> = ({ jobData }) => {
	...
	const fetchTimeWorked = useMemo(
		() =>
			throttle(async () => {
				setLoading(true);
				try {
					let res = await (
						await Nui.send('FetchTimeWorked', jobData.Id)
					).json();
					if (res)
						dispatch({
			...
			const mockTimeWorked = [
				{
					...
					First: 'Testy',
					Last: 'McTest',
					LastClockOn: { [jobData.Id]: now - 3600 },
					TimeClockedOn: {
						[jobData.Id]: [
							{ time: now - 86400, minutes: 95 },
							{ time: now - 172800, minutes: 120 },
							{ time: now - 259200, minutes: 45 },
						],
					},
				},
				{
					...
					First: 'Jane',
					Last: 'Doe',
					LastClockOn: { [jobData.Id]: now - 7200 },
					TimeClockedOn: {
						[jobData.Id]: [
							{ time: now - 86400, minutes: 60 },
							{ time: now - 345600, minutes: 30 },
						],
					},
				},
				{
					...
					First: 'John',
					Last: 'Smith',
					LastClockOn: { [jobData.Id]: now - 604800 },
					TimeClockedOn: {
						[jobData.Id]: [{ time: now - 600000, minutes: 15 }],
					},
				},
			];
			dispatch({
		...
		>
			...
			>
				...
				>
					<FontAwesomeIcon
						icon={['fas', 'clock']}
						style={{ color: T, fontSize: 16, flexShrink: 0 }}
					/>
					<span
						style={{
							overflow: 'hidden',
				...
				<div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
					<div
						...
						onClick={() => fetchTimeWorked()}
						onMouseEnter={(e) => {
							if (!loading) {
								(
									e.currentTarget as HTMLElement
								).style.background = T20;
								(
									e.currentTarget as HTMLElement
								).style.borderColor = T50;
							}
						}}
						onMouseLeave={(e) => {
			...
			<div
				style={{
					...
					overflowX: 'hidden',
					scrollbarWidth: 'thin',
					scrollbarColor: `${T50} transparent`,
				}}
			>
				{loading ? (
				...
				{viewing != null && (
					<div style={{ padding: '8px 0' }}>
						{[
							{
								label: 'Employee',
								value: `${viewing.First} ${viewing.Last}`,
							},
							{ label: 'State ID', value: viewing.SID },
							{
								label: 'Last On Duty',
								value: parseLastDutyTime(
									viewing.LastClockOn,
									jobData.Id,
								),
							},
